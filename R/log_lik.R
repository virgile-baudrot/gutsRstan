...
#'
#'
log_lik.stanTKTD <- function(object,
                             newdata = NULL,
                             ...){
  
  # Observation
  df_obs <- data.frame(Nsurv = object$dataStan$Nsurv,
                       tNsurv = object$dataStan$tNsurv,
                       replicate = object$dataStan$replicate_Nsurv) %>%
    group_by(replicate) %>%
  ...
  mat_psurv <- t(rstan::extract(object$stanfit, pars = c("Psurv_hat"))$Psurv_hat)
