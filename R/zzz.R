#' @useDynLib gutsRstan, .registration = TRUE
#' @import Rcpp
#' @import rstan
#' @import dplyr
...
.onAttach <- function(...) {
  rstanarmLib <- dirname(system.file(package = "gutsRstan"))
  pkgdesc <- suppressWarnings(utils::packageDescription("gutsRstan", lib.loc = rstanarmLib))
  if (length(pkgdesc) > 1) {
    builddate <- gsub(';.*$', '', pkgdesc$Packaged)
    packageStartupMessage(paste("gutsRstan (Version ", pkgdesc$Version, ", packaged: ", builddate, ")", sep = ""))
  }
  ...
  packageStartupMessage("- In addition to functions provided by gutsRstan, we recommend using packages:")
