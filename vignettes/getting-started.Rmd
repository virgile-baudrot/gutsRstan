---
title: "Getting started with rstanguts"
...
a typical use of `rstanguts` on survival data, which can be followed
...
There is only 4 functions in the `rstanguts` package. We want to keep it simple.
...
* Two functions to transform a `stanguts` object into: a `survFit` object to be used with the package [`morse`](https://CRAN.R-project.org/package=morse) ; or a `stanfit` object to use the wonderfull set of packages developped in parallel to the [`rstan`](https://CRAN.R-project.org/package=rstan) package.
...
For instance, within `rstanguts` the default number of chains is 3, so we recommend setting `mc.cores = 3`, if your machine has at least 3 cores which should be the case in a recent computer. 
...
The argument `distribution` matters for `"IT"` or `"PROPER"` models. The default is `"loglogistic"`, and another distribution is `"lognormal"`. If you would like to test some other distribution not implemented, we'll be happy to add any new ones: please let us know by opening a new [github issue](https://github.com/virgile-baudrot/rstanguts/issues).
...
```{r fit_IT_ll, cache=TRUE, echo=TRUE}
...
```{r fit_IT_ln, cache=TRUE, echo=TRUE}
...
```{r fit_PROPER_ll, cache=TRUE, echo=TRUE, message=FALSE}
...
```{r it_PROPER_ln, cache=TRUE, echo=TRUE, message=FALSE}
...
From the R-package [`rstan`](https://CRAN.R-project.org/package=rstan), there is many function to scrutinized the inference process. A simple one is the mcmc:

```{r mcmcRstan, cache=TRUE}
library(rstan)
# mcmc(stanfit_SD_diaz)
# mcmc(stanfit_IT_diaz)
# mcmc(stanfit_IT_lN_diaz)
# mcmc(stanfit_PROPER_lN_diaz)
# mcmc(stanfit_PROPER_ll_diaz)
```

Another is the pairs plot:
...
MF50_SD_diaz <- MFx(survFit_SD_diaz, data_predict = data_4predict, X = 50)
plot(MF50_SD_diaz)
# MF30 at the maximum time-4:
MF30t4_SD_diaz <- MFx(survFit_SD_diaz, data_predict = data_4predict, X = 30, time_MFx = 4)
plot(MF30t4_SD_diaz)

# IT
MF50_IT_diaz <- MFx(survFit_IT_diaz, data_predict = data_4predict, X = 50)
plot(MF50_IT_diaz)
